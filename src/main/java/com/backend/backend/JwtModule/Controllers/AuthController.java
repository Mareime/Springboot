...
public class AuthController {
    ...
    public AuthResponse login(@RequestBody AuthRequest request) {
        ...
        // Get the real user ID based on the role
        Long id;
        switch (role) {
            case "ROLE_PATIENT" -> id = userService.getPatientIdByEmail(email);
            case "ROLE_MEDECIN" -> id = userService.getMedecinIdByEmail(email);
            case "ROLE_ADMIN" -> id = userService.getAdminIdByEmail(email);
            default -> throw new RuntimeException("Rôle inconnu: " + role);
        }
